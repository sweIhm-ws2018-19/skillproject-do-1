...
public class AddToDoOnWeekDayHandler implements RequestHandler {
    ...
    public Optional<Response> handle(HandlerInput input) {
        ...
        if (wochenTag.getValue() != null && wochenTag.getResolutions().toString().contains("ER_SUCCESS_MATCH")) {
            ...
            responseText = String.format("%s wurde zu deiner ToDoListe am %s hinzugef�gt.", toDo.getValue(), wochenTag.getValue());

        } else {

            responseText = "bitte Wochentag nennen, an dem das ToDo hinzugef�gt werden soll.";
